...
public class Shop2 {
    ...
    public static boolean check(Char charz, Shop shop, Item item) {
        ...
        //Diem vip
        if (item.iconSpec == 6689) {
            if (charz.pointVip < item.buySpec) {
                charz.addInfo1(String.format(mResources.CAN_ITEM, item.buySpec - charz.pointVip, "Điểm"));
                return true;
            }
        }
        return false;
